...
func (k Keeper) FundModAccForInterest(ctx sdk.Context, poolID, assetID uint64, lenderAddr sdk.AccAddress, payment sdk.Coin) error {
	pool, found := k.GetPool(ctx, poolID)
	if !found {
		return types.ErrPoolNotFound
	}
	...
	poolAssetLBMappingData, _ := k.GetAssetStatsByPoolIDAndAssetID(ctx, poolID, assetID)
	poolAssetLBMappingData.TotalInterestAccumulated = poolAssetLBMappingData.TotalInterestAccumulated.Add(payment.Amount)
	k.SetAssetStatsByPoolIDAndAssetID(ctx, poolAssetLBMappingData)
